...
class AlloyCalculator {
    ...
    this.percentStep = Math.pow(10, -this.percentPrecision);
        ...
        this.isReady = Boolean(this.container && this.alloySelect && this.ingotsInput);
        if (!this.isReady) {
    ...
    selectAlloy(key) {
        ...
        this.state.parts = definition.parts.map(part => {
            ...
            const max = Number.isFinite(part.max) && part.max >= min ? part.max : Math.max(min, 100);
    ...
    renderBar() {
        ...
        const totalPercent = this.state.parts.reduce((sum, part) => sum + part.pct, 0);
        if (totalPercent <= 0) {
    ...
    rebalancePercents(anchorIndex = 0) {
        ...
        const capacity = entry => direction > 0
        ...
        if (remaining > 1e-4 && parts[anchorIndex]) {
            ...
            const anchorCapacity = direction > 0 ? anchor.max - anchor.pct : anchor.pct - anchor.min;
        ...
        if (Math.abs(finalDiff) > 0.01 && parts[anchorIndex]) {
            parts[anchorIndex].pct = clamp(parts[anchorIndex].pct + finalDiff, parts[anchorIndex].min, parts[anchorIndex].max);
        }
    }
    ...
    roundParts(anchorIndex = 0) {
        ...
        parts.forEach(part => {
            part.pct = clamp(Number(part.pct.toFixed(this.percentPrecision)), part.min, part.max);
        });

        let diff = Number((100 - parts.reduce((sum, part) => sum + part.pct, 0)).toFixed(this.percentPrecision));
        ...
        const candidates = parts
            ...
            .filter(entry => entry.idx !== anchorIndex)
            .sort((a, b) => {
                const capA = direction > 0 ? a.part.max - a.part.pct : a.part.pct - a.part.min;
                const capB = direction > 0 ? b.part.max - b.part.pct : b.part.pct - b.part.min;
                return capB - capA;
            });

        const target = candidates.find(entry => {
            const cap = direction > 0 ? entry.part.max - entry.part.pct : entry.part.pct - entry.part.min;
            return cap > 0;
        }) || { part: parts[anchorIndex], idx: anchorIndex };

        if (!target.part) return;
        const capacity = direction > 0 ? target.part.max - target.part.pct : target.part.pct - target.part.min;
        const delta = direction * Math.min(Math.abs(diff), capacity);
        target.part.pct = clamp(Number((target.part.pct + delta).toFixed(this.percentPrecision)), target.part.min, target.part.max);

        const epsilon = Math.pow(10, -this.percentPrecision);
        let finalDiff = Number((100 - parts.reduce((sum, part) => sum + part.pct, 0)).toFixed(this.percentPrecision));
        if (Math.abs(finalDiff) >= epsilon) {
            ...
            let adjustIndex = parts.findIndex((part, idx) => {
                ...
                const cap = adjustDirection > 0 ? part.max - part.pct : part.pct - part.min;
                return cap >= epsilon;
            });
            if (adjustIndex === -1) adjustIndex = anchorIndex;
            const adjustPart = parts[adjustIndex];
            if (adjustPart) {
                const cap = adjustDirection > 0 ? adjustPart.max - adjustPart.pct : adjustPart.pct - adjustPart.min;
                const adjustment = adjustDirection * Math.min(Math.abs(finalDiff), cap);
                adjustPart.pct = clamp(Number((adjustPart.pct + adjustment).toFixed(this.percentPrecision)), adjustPart.min, adjustPart.max);
                finalDiff = Number((100 - parts.reduce((sum, part) => sum + part.pct, 0)).toFixed(this.percentPrecision));
                if (Math.abs(finalDiff) >= epsilon && adjustIndex !== anchorIndex) {
                    const anchor = parts[anchorIndex];
                    if (anchor) {
                        const anchorCap = adjustDirection > 0 ? anchor.max - anchor.pct : anchor.pct - anchor.min;
                        const anchorAdjustment = adjustDirection * Math.min(Math.abs(finalDiff), anchorCap);
                        anchor.pct = clamp(Number((anchor.pct + anchorAdjustment).toFixed(this.percentPrecision)), anchor.min, anchor.max);
                    }
                }
            }
        }
    ...
    calculatePieceAllocations(totalUnits) {
        ...
        const totalPieces = Math.max(0, Math.round(totalUnits / this.UNITS_PER_PIECE));
        if (totalPieces === 0) {
        ...
        let assignedPieces = allocations.reduce((sum, entry) => sum + entry.pieces, 0);
...
if (typeof document !== 'undefined') {
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => new AlloyCalculator(document));
    } else {
